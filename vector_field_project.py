...
Original file is located at
    https://colab.research.google.com/drive/1YVT_M4mVr7jmENhtHBEjt1HEHjRDMTUj
"""

import numpy as np
from scipy.integrate import quad

# Define the functions for the vector field and the parameterization of the curve
def f(t):
    return np.array([4*t*np.cos(t), -np.cos(t)**2, -2*np.sin(t)])

def r(t):
    return np.array([np.sin(t), np.cos(t), t])

# Define the integrand function for the dot product of f and r'
def integrand(t):
    return np.dot(f(t), np.array([np.cos(t), -np.sin(t), 1]))

# Evaluate the integral using numerical integration
result, _ = quad(integrand, 0, np.pi)

print("Result of the line integral:", result)
...
# copy the filepath
os.chdir("C:\\Users\\Qianx\\Downloads") # Changed the path to make it valid
...
df = pd.read_csv('Nat_Gas.csv', parse_dates=['Dates']) # Changed filename to match the directory change
